...
async Hitmeckapi(e) {
    ...
    await this.e.reply('正在访问接口生成图片，请耐心等待...')
    let msg = [
        ["真寻https://mahiro.tianyi.one\n",segment.image(`https://mahiro.tianyi.one`),],["二次元壁纸https://t.mwm.moe/pc\n",segment.image(`https://t.mwm.moe/pc`),],["二次元风景https://t.mwm.moe/fj\n",segment.image(`https://t.mwm.moe/fj`),],["随机ai图https://t.mwm.moe/ai\n",segment.image(`https://t.mwm.moe/ai`),],["动漫http://www.dmoe.cc/random.php\n",segment.image(`http://www.dmoe.cc/random.php`),],["随机图https://api.yimian.xyz/img\n",segment.image(`https://api.yimian.xyz/img`),],["随机可爱图http://www.98qy.com/sjbz/api.php\n",segment.image(`http://www.98qy.com/sjbz/api.php`),],["随机竖屏图https://api.anosu.top/img\n",segment.image(`http://www.98qy.com/sjbz/api.php`),],["原神https://api.dujin.org/pic/yuanshen\n",segment.image(`https://api.dujin.org/pic/yuanshen`),],["随机pc涩图ph.tianyi.one/pc.php\n",segment.image(`ph.tianyi.one/pc.php`),],["随机r18图ph.tianyi.one/rd.php\n",segment.image(`ph.tianyi.one/rd.php`),],["随机萝莉r18图http://loli.tianyi.one\n",segment.image(`http://loli.tianyi.one`),],["使用方法：\n#写入打人api+上面的随便一个api即可\n注意每次更换api时请先 #删除打人api 再写入api"]
    ]
    e.reply(common.makeForwardMsg(e,msg,`小寻`,true))
    return true
    }
