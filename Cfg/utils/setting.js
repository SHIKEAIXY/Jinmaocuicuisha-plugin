...
class Setting {
  constructor () {
    this.defPath = `${_path}/plugins/Jinmaocuicuisha-plugin/Cfg/`
    this.defSet = {}

    this.configPath = `${_path}/plugins/Jinmaocuicuisha-plugin/Cfg/`
    this.config = {}
  ...
  getFilePath (app, type) {
    if (type === 'defSet') return `${this.defPath}${app}.yaml`
      ...
      try {
        ...
        logger.error(`脆脆鲨插件缺失默认文件[${app}]${error}`)
      }
  ...
  watch (file, app, type = 'defSet') {
    ...
    watcher.on('change', path => {
      ...
      logger.mark(`[脆脆鲨插件][修改配置文件][${type}][${app}]`)
      if (this[`change_${app}`]) {
        this[`change_${app}`]()
      }
    })
